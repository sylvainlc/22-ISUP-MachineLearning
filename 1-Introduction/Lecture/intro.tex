...
$\rightharpoondown$  $(X,Y)\in\mathbb{R}^d\times \{1,\ldots,M\}$ and the objective is to \alert{define a function $f: \mathbb{R}^d \to \{1,\ldots,M\}$}, called classifier, such that $f(X)$ is the best prediction of $Y$ in a given context.
...
$\rightharpoondown$ $(X,Y)\in\mathbb{R}^d\times \mathbb{R}^m$ and \alert{the objective is to define the  best estimator of $f_*$ in a given context}. %An element of $\mathbb{R}^p$ contains all the features the label prediction or the regression estimate has to be based on.
\end{frame}
